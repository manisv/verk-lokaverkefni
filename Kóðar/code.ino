...
void loop () {
  ...
  switchOne = digitalRead(4);
  switchTwo = digitalRead(5);
  switchThree = digitalRead(6);
  ...
  if(fjarlaegd < 100 and switchOne == HIGH) { // fjarlægð getur ekki verið mínustala
    fjarlaegd = 0;
    sing(1);
  }  
  if (fjarlaegd < 100 and switchTwo == HIGH) {
    fjarlaegd = 0;
    sing(2);
  } 
  if (fjarlaegd < 100 and switchThree == HIGH) {
    fjarlaegd = 0;
    sing(3);
  }
